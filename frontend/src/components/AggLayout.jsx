...
export default function AggLayout({ children }) {
  ...
  const activeCategory = loc.pathname.startsWith("/category/") ? loc.pathname.split("/")[2] : null;
    ...
    <div className="min-h-screen flex flex-col agg-bg">
      <header className="bg-agg-navy text-agg-cream border-b border-agg-navy-deep">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between py-3">
            <Link to="/" data-testid="agg-nav-home" className="flex items-center gap-3 group">
              <span className="font-display font-semibold tracking-tight text-lg sm:text-xl text-agg-cream">
                The Housing News
              </span>
            ...
            <div className="flex items-center gap-5">
              <Link to="/newsletter" data-testid="agg-nav-newsletter" className="hidden sm:inline font-sans text-sm font-medium text-agg-cream hover:text-agg-orange transition-colors">
                Newsletter
              </Link>
              <Link to="/about" data-testid="agg-nav-about" className="hidden sm:inline font-sans text-sm font-medium text-agg-cream hover:text-agg-orange transition-colors">
                About
              </Link>
              <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
                ...
                >
                  ...
                  <nav className="flex flex-col px-2 pb-6">
                    <Link to="/" onClick={close} className="font-display text-lg px-4 py-3 rounded-sm hover:bg-agg-navy-deep hover:text-agg-orange transition-colors">Home</Link>
                    {CATEGORIES.filter(c => c.key).map(c => (
                      <Link key={c.key} to={`/category/${c.key}`} onClick={close} className="font-display text-lg px-4 py-3 rounded-sm hover:bg-agg-navy-deep hover:text-agg-orange transition-colors">
                        {c.label}
                      </Link>
                    ))}
                    <Link to="/newsletter" onClick={close} className="font-display text-lg px-4 py-3 rounded-sm hover:bg-agg-navy-deep hover:text-agg-orange transition-colors">Newsletter</Link>
                    <Link to="/about" onClick={close} className="font-display text-lg px-4 py-3 rounded-sm hover:bg-agg-navy-deep hover:text-agg-orange transition-colors">About</Link>
                    ...
                    <div className="border-t border-agg-navy-deep mt-3 pt-3">
                      <Link to="/welcome" onClick={close} className="block font-sans text-sm px-4 py-2 text-agg-cream/70 hover:text-agg-orange transition-colors">
                        Members club →
                      </Link>
                    </div>
                  </nav>
          ...
          <nav className="flex items-center gap-1 sm:gap-2 overflow-x-auto -mx-1 pb-2 sm:pb-3 text-sm" data-testid="agg-category-filter">
            {CATEGORIES.map(c => {
              const to = c.key === null ? "/" : `/category/${c.key}`;
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-sm text-slate-600">
          ...
          <div className="flex items-center gap-5">
            <Link to="/about" className="hover:text-agg-orange transition-colors">About</Link>
            <Link to="/newsletter" className="hover:text-agg-orange transition-colors">Newsletter</Link>
            <a href="/legal/terms-of-service.pdf" target="_blank" rel="noreferrer" className="hover:text-agg-orange transition-colors">Terms</a>
            <Link to="/welcome" className="hover:text-agg-orange transition-colors">Members</Link>
          </div>
        </div>
      </footer>
    </div>
